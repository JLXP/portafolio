...
export const Cursos = () => {
    ...
    function toggleModal(diploma) {
        if (imgRef.current.ownerDocument.body.clientWidth < 780) return null;
        if (open) {
            setImg(null)
    ...
    return (
        <section className="w-full h-full px-6" id="cursos">
                ...
                <div className="w-full flex items-center justify-center">
                    <div className=" w-4/5 flex justify-center items-center flex-wrap">
                        {diplomas.map((diploma,index) => (
                            <div className="w-[300px] h-[250px] m-2 cursor-pointer" disabled={false} onClick={() => toggleModal(diploma)} ref={imgRef} key={index}>
                                <div className="flex justify-center items-center hover:opacity-60 hover:scale-110 duration-500 w-full h-full bg-black rounded-lg relative">
                                    <img className="rounded-lg h-full w-full image-cover" loading='lazy' src={diploma.src} />
                                </div>
                            </div>
                        ))}
                    </div>
